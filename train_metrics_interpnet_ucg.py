...
for name, manifold in metrics.items():
    model_path = os.path.join(save_dir, f"{name}_geodesic_ucg.pt")
...
plot_geodesics_on_gmm_density(
    ...
    X=X,
    Y=Y,
    title="Geodesics on Semicircle Gaussian Mixture",
    save_path="plots/gaussian_blobs/exp5/geodesics_semicircle.png",
    device=device
)